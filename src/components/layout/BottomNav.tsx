...
import { LayoutDashboard, CheckSquare, Calendar, Wallet, Activity } from 'lucide-react';
...
export default function BottomNav() {
  ...
  return (
    <nav className="fixed bottom-4 left-4 right-4 z-50 max-w-lg mx-auto">
      <div className="glass-card rounded-full border border-white/15 shadow-[0_15px_35px_rgba(0,0,0,0.6)] px-1.5 py-1.5 flex items-center justify-around backdrop-blur-2xl">
        {navItems.map((item) => {
          const isActive =
            item.href === '/'
              ? optimisticPath === '/'
          ...
          return (
            <Link
              ...
              className={`flex items-center gap-1 px-3 py-2 rounded-full transition-all duration-150 active:scale-95 ${
                isActive
